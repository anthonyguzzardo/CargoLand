...
let hasCargo : boolean = true;
let pumpActive : boolean = false;
let waterLevel : number = 1; // 1 = full, 0 = drained
const keys = new Set<string>();
const trees: Tree[] = [];

// Drone position in world coordinates
let droneX : number = 100;
let droneY : number = 100;
...
function checkTreeCollision(newX: number, newY: number): boolean {
...
function isNearDropZone(x: number, y: number): boolean {
...
function dropCargo() {
    if (!hasCargo || !world || !drone) return;
...
function update(){
    ...
    if(keys.has('KeyA')) newX -= 5;
    if(keys.has('KeyD')) newX += 5;
    if(keys.has('KeyW')) newY -= 5;
    if(keys.has('KeyS')) newY += 5;
